...
export default function AdminVisitors() {
  ...
  useEffect(() => { fetchVisitors(); fetchEmailSetting(); }, []);
  ...
  const stats = useMemo(() => {
    ...
    // Unique IPs
    const ipCounts = {};
    visitors.forEach(v => { if (v.ip) ipCounts[v.ip] = (ipCounts[v.ip] || 0) + 1; });
    const uniqueIPs = Object.keys(ipCounts).length;
    const topIPs = Object.entries(ipCounts).sort((a, b) => b[1] - a[1]).slice(0, 8);

    // Today's visitors
    const todayCount = visitors.filter(v => v.time && v.time.startsWith(today)).length;

    // This week
    const weekAgo = new Date(now); weekAgo.setDate(weekAgo.getDate() - 7);
    const weekCount = visitors.filter(v => v.time && new Date(v.time) >= weekAgo).length;
    ...
    // Browsers
    const browserCounts = {};
    visitors.forEach(v => { const b = v.browser || 'Unknown'; browserCounts[b] = (browserCounts[b] || 0) + 1; });
    const topBrowsers = Object.entries(browserCounts).sort((a, b) => b[1] - a[1]);

    // OS
    const osCounts = {};
    visitors.forEach(v => { const o = v.os || 'Unknown'; osCounts[o] = (osCounts[o] || 0) + 1; });
    const topOS = Object.entries(osCounts).sort((a, b) => b[1] - a[1]);

    // Referrers
    const refCounts = {};
    visitors.forEach(v => { const r = v.referrer || 'Direct'; refCounts[r] = (refCounts[r] || 0) + 1; });
    ...
    // Screens
    const screenCounts = {};
    visitors.forEach(v => { if (v.screen) screenCounts[v.screen] = (screenCounts[v.screen] || 0) + 1; });
    ...
    return {
      total: visitors.length, uniqueIPs, todayCount, weekCount,
  ...
  return (
    <div style={S.container}>
      ...
      <div style={{ display: 'flex', gap: '0', marginBottom: '1.5rem', borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--border-color)' }}>
        {[['stats', '📊 Statistics'], ['history', '📋 History']].map(([key, label]) => (
      ...
      {visitors === null ? (
        <div className='text-center py-5'>
          ...
          <p className='mt-3' style={{ color: 'var(--text-secondary)' }}>Loading visitors...</p>
        </div>
      ) : visitors.length === 0 ? (
        <div className='text-center py-5'>
